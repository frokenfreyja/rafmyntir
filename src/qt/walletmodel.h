...
public:
    ...
    explicit SendCoinsRecipient(const QString &addr, const QString &label, const QString &data, quint64 amount, const QString &message):
        address(addr), label(label), data(data), amount(amount), message(message), nVersion(SendCoinsRecipient::CURRENT_VERSION) {}
